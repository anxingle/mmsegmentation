...
model = dict(
    ...
    decode_head=dict(
        ...
        loss_decode=[
            dict(type='CrossEntropyLoss', use_sigmoid=False, loss_weight=0.5, loss_name='loss_ce', avg_non_ignore=True),
            dict(type='DiceLoss', eps=1e-3, naive_dice=False, use_sigmoid=False, loss_weight=1.6, loss_name='loss_dice'),
        ],
    ),
    auxiliary_head=dict(
        type='FCNHead',
        ...
        norm_cfg=_NORM_CFG, align_corners=False, ignore_index=255,
        loss_decode=[
            dict(type='CrossEntropyLoss', use_sigmoid=False, loss_weight=0.1, loss_name='loss_ce_aux', avg_non_ignore=True),
            dict(type='DiceLoss', eps=1e-3, naive_dice=False, use_sigmoid=False, loss_weight=0.22, loss_name='loss_dice_aux'),
        ],
    ),
    train_cfg=dict(),
...
work_dir = './work_dirs/AN_UNet_1024_acc_optimizer'
